...
const BlogHome = ({
  ...
  const [posts, setPosts] = React.useState<Post[]>(incomingPosts);
  ...
  return (
    <div className="flex flex-col items-start justify-start min-h-full bg-white p-4 sm:p-6 lg:p-8 overflow-y-auto">
        <div className="mx-auto w-full max-w-6xl 2xl:max-w-7xl">
          <BlogHeader allPosts={incomingPosts} />
          <div className="my-6 sm:my-8 w-full">
            <BlogCategories
              selectedCategory={selectedCategory}
              clickHandler={clickedCategory}
              categories={formattedCategories}
            />
          </div>
          {selectedCategory !== "All" && posts?.length === 0 && (
            <p className="col-span-2 flex items-center justify-center text-center text-sm text-gray-600 py-12">
              <RiEmotionUnhappyLine size={20} className="mr-2" /> No blogs found
              for this category.
            </p>
          )}
          {posts?.length > 0 && (
                ...
                <div className="mx-auto mb-16 grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
                  {visiblePosts.map((item) => (
                    <BlogCard
                      ...
                      title={item.title}
                      tags={item?.tags?.nodes?.map((tag) => tag?.name)}
              ...
              {!isExpanded && selectedCategory === "All" && (
                <div className="text-center mb-12">
                  <button
                    ...
                    className="bg-[#6366f1] hover:bg-[#5558e3] text-white font-medium px-8 py-3 rounded-lg transition-colors duration-200 shadow-sm hover:shadow-md"
                  >
                    View All Posts
                  </button>
                </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between gap-8 md:gap-12 mb-12 md:mb-20">
            ...
            <div>
              <div className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-3">
                ...
                />
                <span className="text-lg md:text-xl text-[#6366f1] font-normal">Conch</span>
              </div>
            ...
            <div className="flex gap-16 md:gap-24">
              {/* About */}
              <div>
                <p className="text-[10px] md:text-xs text-muted-foreground uppercase tracking-wider mb-3 md:mb-5 font-normal">About</p>
                <div className="flex flex-col gap-2.5 md:gap-4">
                  <button 
                    onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
                    className="text-[13px] md:text-base text-foreground hover:text-[#6366f1] transition-colors font-normal text-left"
                  >
                    Blog
                  </button>
                  <a href="/terms" className="text-[13px] md:text-base text-foreground hover:text-[#6366f1] transition-colors font-normal">Terms of Service</a>
                  <a href="/privacy" className="text-[13px] md:text-base text-foreground hover:text-[#6366f1] transition-colors font-normal">Privacy Policy</a>
                </div>
              </div>
              {/* Support */}
              <div>
                <p className="text-[10px] md:text-xs text-muted-foreground uppercase tracking-wider mb-3 md:mb-5 font-normal">Support</p>
                <div className="flex flex-col gap-2.5 md:gap-4">
                  <a href="mailto:help@getconch.ai" className="text-[13px] md:text-base text-foreground hover:text-[#6366f1] transition-colors font-normal">Contact Us</a>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Copyright */}
          <div>
            <p className="text-[12px] md:text-sm text-muted-foreground font-normal">Yofi Tech, LLC • Copyright © 2026</p>
          </div>
        </div>
      </footer>
        </div>
